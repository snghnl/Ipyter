...
module TEnv = struct
  ...
  let rec init_tEnv : identifier -> tEnv -> tEnv
...
module TEnvMap = struct   
  type key = TEnv.key
  type value = constant list
  type tenv = value TEnv.t 

  let rec init_tEnv : key -> tenv -> tenv
    = fun key tenv -> TEnv.filter (fun x  -> key = x)
...
module TEnv = struct
  ...
  let rec init_tEnv : identifier -> CandVars.t -> t
  = fun var candvars -> 
    
    List.filter (fun (a, _) -> a <> var) candvars

  let rec infer: identifier -> t -> modul -> constant list list
  = fun var tenv pgm ->  
      match pgm with 
