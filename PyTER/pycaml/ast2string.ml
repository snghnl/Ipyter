(* Clone code of https://github.com/hakjoooh/PyIR/blob/main/frontend/ast2string.ml *)
open Ast
open Util 
...
and string_of_stmt indent stmt : string =
  ...
  | If a -> string_of_while_stmt indent a.test a.body a.orelse
...
and string_of_functiondef_stmt indent name args body returns decorator_list is_async = 
  string_of_list (fun e -> tab indent ^ "@" ^ string_of_expr e ^ "\n") decorator_list ~first:"" ~last:"" ~sep: "" ^ tab indent ^ (if is_async then "async " else "") ^ "def " ^ 
...
and string_of_with_stmt indent items body is_async = 
      tab indent ^ (if is_async then "async " else "") ^ "with " ^ string_of_list string_of_withitem items 
      ~first: "" ~last: "" ~sep: ", " ^ ":\n" ^
      string_of_stmts (indent + 1) body 
...
and string_of_match_stmt indent subject cases = 
    tab indent ^ "match " ^ string_of_expr subject ^ ":\n" ^ 
      string_of_list (fun (Match_case case) -> 
        string_of_matchcase (indent + 1) case.pattern case.guard case.body) cases ~first: "" ~last: "" ~sep: ""
...
and string_of_importfrom_stmt indent modul names _ = 
